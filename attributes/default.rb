...
default[:aem][:commands] = {
  :replicators => {
    :publish => { :add => 'curl -u <%=local_user%>:<%=local_password%> -X POST http://localhost:<%=local_port%>/etc/replication/agents.author/<%=aem_instance%><%=instance%>/_jcr_content -d jcr:title="<%=type%> Agent <%=instance%>" -d transportUri=http://<%=h[:ipaddress]%>:<%=h[:port]%>/bin/receive?sling:authRequestLogin=1 -d enabled=true -d transportUser=<%=h[:user]%> -d transportPassword=<%=h[:password]%> -d cq:template="/libs/cq/replication/templates/agent" -d retryDelay=60000 -d logLevel=info -d serializationType=durbo -d jcr:description="<%=type%> Agent <%=instance%>" -d sling:resourceType="cq/replication/components/agent"'},
    :flush => {:add => 'curl -u <%=local_user%>:<%=local_password%> -X POST http://localhost:<%=local_port%>/etc/replication/agents.publish/flush<%=instance%>/_jcr_content  -d transportUri=http://<%=h[:ipaddress]%>/dispatcher/invalidate.cache -d enabled=true -d transportUser=<%=h[:user]%> -d transportPassword=<%=h[:password]%>'},
    :agent => {:add => 'curl -F "jcr:primaryType=cq:Page" -F "jcr:content=" -u <%=local_user%>:<%=local_password%> http://localhost:<%=local_port%>/etc/replication/agents.author/<%=aem_instance%><%=instance%>',
               :remove => 'curl -u <%=local_user%>:<%=local_password%> -X DELETE http://localhost:<%=local_port%>/etc/replication/agents.author/<%=h%>',
...
default[:aem][:dispatcher][:webserver_type] = "apache#{node['apache']['version']}"
default[:aem][:dispatcher][:base_dir] = "/opt/dispatcher"
default[:aem][:dispatcher][:conf_file] = "#{node['aem']['dispatcher']['base_dir']}/conf/dispatcher.any"
default[:aem][:dispatcher][:log_file] = "#{node['aem']['dispatcher']['base_dir']}/logs/dispatcher.log"
default[:aem][:dispatcher][:log_level] = "1"
default[:aem][:dispatcher][:farm_dir] = "#{node['aem']['dispatcher']['base_dir']}/conf/aem-farms"
